<x-app-layout>
        ...
        <div class="flex h-screen">
            ...
            <div class="flex flex-col flex-grow">
                ...
                <div class="main-content">
                    <h1 class="text-2xl font-light text-center mb-4" style="color: #007b3e;">Search Results for Phone Number: {{ $phone_number }}</h1>
                    <div class="flex">
                        <div id="graph" class="flex-1"></div>
                        <div class="right-sidebar">
                            <h2 class="text-xl font-semibold mb-2" style="color: #1eb53a;">Call Data</h2>
                            @foreach($Call as $call)
                                <div class="border rounded p-2 mb-2">
                                    @foreach($call->toArray() as $key => $value)
                            ...
                            @endforeach
                            <h2 class="text-xl font-semibold mb-2" style="color: #007b3e;">Civil Register Data</h2>
                            @foreach($CivilRegister as $record)
                                <div class="border rounded p-2 mb-2">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const data = {
                nodes: [
                    { id: '{{ $phone_number }}', group: 1 },
                    @foreach($CivilRegister as $record)
                        { 
                            id: 'Civil Register: {{ $record->id }}', 
                            group: 2,
                    ...
                    @foreach($Call as $call)
                        { 
                            id: 'Call Log: {{ $call->id }}', 
                            group: 3,
                            details: {!! json_encode(array_diff_key($call->toArray(), ['created_at' => '', 'updated_at' => ''])) !!}
                        },
                    @endforeach
                ],
                links: [
                    @foreach($CivilRegister as $record)
                        { source: '{{ $phone_number }}', target: 'Civil Register: {{ $record->id }}', value: 1 },
                    @endforeach
                    @foreach($Call as $call)
                        { source: '{{ $phone_number }}', target: 'Call Log: {{ $call->id }}', value: 1 },
                    @endforeach
                ]
            };
